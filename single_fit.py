...
def  main():
    ...
    for syst_cat in syst_cats:
        ...
        fig, ax = plt.subplots(figsize=(7, 1 + num_nps / 4), dpi=100,layout='tight')
        ...
        ax.errorbar(bestfits, y_positions, xerr=bestfits_unc, mfc='none', mec='none', ecolor=pull_colors, linestyle='None')
        ax.fill_between([-2, 2], -0.5, len(bestfits) - 0.5, color="#ffd166")#"#ebf5df")
        ax.fill_between([-1, 1], -0.5, len(bestfits) - 0.5, color="#06d6a0")#"#bad4aa")
        ax.vlines(0, -0.5, len(bestfits) - 0.5, linestyles="dotted", color="black")

        ax.scatter(bestfits, y_positions, marker="o", s=30, color=pull_colors, linestyle='None', alpha=0.8)
        # X-axis
        ax.set_xlim([-3, 3])
        ax.set_xlabel(r"$\left(\hat{\theta} - \theta_0\right) / \Delta \theta$", fontsize=LABEL_SIZE-2)
        ax.xaxis.set_minor_locator(AutoMinorLocator())

        # Y-axis
        ax.set_ylim([-0.5, num_nps - 0.5])
        ax.set_yticks(y_positions)
        ax.set_yticklabels(labels)

        # Tick params
        ax.tick_params(axis="both", which="major", pad=8, labelsize=LABEL_SIZE-4)
        ax.tick_params(direction="in", top=True, right=True, which="both")

        # # Save it!
        fig.savefig(f"{outdir}/pulls/pulls_{syst_cat_name}.pdf", dpi=300, bbox_inches='tight')
